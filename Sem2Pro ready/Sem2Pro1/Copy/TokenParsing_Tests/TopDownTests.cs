...
namespace TokenParsing_Tests
{
    public class TopDownTests
    {
        ...
        public void TestSimple(string input, Expression expected)
        {
            ...
            var prStr = new PrioritizedString(input);
            var parsed = new ParserCombinator().Parse(prStr, userInput);
            Assert.True(ExpressionComparer.AreEqual(parsed, expected));
        }
        ...
        public void TestCall(string input, Expression exprected)
        {
            var combinator = new ParserCombinator();
            var userInput = new UserInput(input);
            var actual = combinator.Parse(userInput.FullInput, userInput);
            Assert.True(ExpressionComparer.AreEqual(actual, exprected));
        }
    }
}